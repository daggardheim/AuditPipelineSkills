...
| S2 | Auditor | Audit the runbook against template rules (read-only) |
| S3 | Creator | Rewrite the runbook to fix S2 findings, verifying facts against source material |
| S4 | Auditor | Confirm only the changes from S3 (read-only) |
...
5. When S5 is complete:
   ...
   - Set the next row's S1 from `not-started` to `todo`
...
- source material paths (from domain config)
